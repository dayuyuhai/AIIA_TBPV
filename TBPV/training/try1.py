...
class ViT(nn.Module):
    def __init__(self, pc_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels, dim_head, dropout, emb_dropout, group):
        super().__init__()
        ...
        num_of_block = patch_height * patch_width * patch_length
        # 在进入模型之前，把这几个组块拼在一起了
        pc_height = pc_height * (num_of_group)

        print("pc length,height, width:", pc_length, pc_height, pc_width)
        print("patch length, height, width: ",
              patch_length, patch_height, patch_width)
        # 一个组的维数
        ...
        self.to_patch_embedding = nn.Sequential(
            ...
            Rearrange('b n c -> b (n c) 1',
                      n = num_of_group, c=patch_dim),
            nn.Linear(1, dim),
        )

        self.pos_embedding = nn.Parameter(torch.randn(1, num_of_block * 8 + 1, dim))
...
"""
有改动，group取几就抠出几组的index
"""
def get_index(group, d, h, w):

    index = [[np.arange(0, d, 2), np.arange(0, h, 2), np.arange(0, w, 2)],
             [np.arange(0, d, 2), np.arange(0, h, 2), np.arange(1, w, 2)],
             [np.arange(0, d, 2), np.arange(1, h, 2), np.arange(0, w, 2)],
             [np.arange(0, d, 2), np.arange(1, h, 2), np.arange(1, w, 2)],
             [np.arange(1, d, 2), np.arange(0, h, 2), np.arange(0, w, 2)],
             [np.arange(1, d, 2), np.arange(0, h, 2), np.arange(1, w, 2)],
             [np.arange(1, d, 2), np.arange(1, h, 2), np.arange(0, w, 2)],
...
def train(use_cuda, batch_size, low_res, max_epochs, group, output_model, dataset_path, valid_loss_min, model,
    ...
    for epoch in range(start_epoch, max_epochs):
        for batch_idx, x in enumerate(training_generator):
            ...
            input = maxpool_n(x)
            # print(type(input_))
            # print('Input shape(before): ', input_.shape) # b 1 lowres * 3

            # 输入当前大小的块块
            """
            对于第0组，输入只有祖先块，其他组会输入祖先块之前的组块
            """
            if group != 0:
                init_input = maxpool_n1(x)

            for i in range(group):
                index = get_index(i, block_size, block_size, block_size)
                tmp_input = init_input[:, :, index[0][:,None, None], index[1][:, None], index[2]]
                input = torch.cat([input, tmp_input], dim=2)
        ...
        for batch_idx, x in enumerate(valid_generator):
            ...
            for i in range(group):
                ...
                tmp_input = init_input[:, :, index[0][:, None, None], index[1][:, None], index[2]]
                input = torch.cat([input, tmp_input], dim=2)
                # input = np.concatenate((input, tmp_input), axis=2)
...
if __name__ == "__main__":
    ...
    model = ViT(low_res, low_res, 2, args.dim, args.depth, args.heads, args.mlp_dim, 1, 64, 0.1, 0.1, args.group)
