...
from training.ms_voxel_transformer_training import get_index, ViT
...
def encoder(args):
    ...
    for lv in range(downsample_level):
        for gr in range(8):
            ...
            model = ViT(low_res, low_res, 2, 1024, 3, 2, 512, 1, 64, 0.1, 0.1, gr)
...
def predictor(curr_block, ds_block, curr_lv, group):
    ...
    for i in range(group):
        index = get_index(i, d, w, h)
        tmp_input = curr_block[:, :, index[0][:,None, None], index[1][:, None], index[2]]
        input_ = torch.cat([input_, tmp_input], dim=2)

    input_ = input_.to(device)
    group_prediction = Models[curr_lv * 8 + group](input_)
...
if __name__ == "__main__":
    ...
    # 下采样深度 3
    parser.add_argument("-ds_depth", '--downsamplingdepth', type=int,
                        default=3,
...
# python3 -m  ms_voxel_dnn_coder.transformer_based_encoder -level 10 -ds_depth 3 -ply ./datasets_bp/MPEG/10bitdepth/train/longdress_vox10_1052.ply -output Output/ -signaling tbpv -model Model/TBPVoxel/ -model8 ./Model/voxelDNNBL8_tf0/

# python3 -m  ms_voxel_dnn_coder.transformer_based_encoder -level 10 -ds_depth 3 -ply ./datasets_bp/Microsoft/10bitdepth/train/frame0004.ply -output Output/ -signaling tbpv -model Model/TBPVoxel/ -model8 ./Model/voxelDNNBL8_tf0/
