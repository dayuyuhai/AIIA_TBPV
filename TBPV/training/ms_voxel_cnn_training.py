...
class ResnetBlock(nn.Module):
    def __init__(self, in_filters, out_filters, *args, **kwargs):
        ...
        self.block = nn.Sequential(
            conv_bn(self.in_filters, self.out_filters,
                    kernel_size=1, stride=1, padding=0, bias=False),
            nn.ReLU(),
            conv_bn(self.out_filters, self.out_filters,
                    kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(),
            conv_bn(self.out_filters, self.in_filters,
                    kernel_size=1, stride=1, padding=0, bias=False),
        )
...
class ResNet_kkk(nn.Module):
    def __init__(self, in_filters, out_filters, nblocks, block):
        ...
        self.blocks = nn.Sequential(
            nn.Conv3d(self.in_filters, self.out_filters,
...
class ResNet_kk2k(nn.Module):
    def __init__(self, in_filters, out_filters, nblocks, block):
        ...
        self.block = block
        self.blocks = nn.Sequential(
            nn.Conv3d(self.in_filters, self.out_filters, kernel_size=7,
                      stride=1, padding=(3, 3, 3), bias=False),
            nn.Conv3d(self.out_filters, self.out_filters, kernel_size=3,
                      stride=(1, 1, 2), padding=(1, 1, 1), bias=False),
            *[block(self.out_filters, self.out_filters) for _ in range(self.nblocks)]
        )
...
class VoxelCNN(nn.Module):
    def __init__(self, input_channel):
        ...
        self.pixelcnn = nn.Sequential(
            maskedConv3D(masktype='A', in_channels=input_channel,
                         out_channels=64, kernel_size=7, stride=1, padding=3),
            maskedResnet(32),
            maskedConv3D(masktype='B', in_channels=64,
                         out_channels=32, kernel_size=1, stride=1, padding=0),
            nn.ReLU(),
            maskedConv3D(masktype='B', in_channels=32,
...
class MSVoxelCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = ResnetFeature.unfold(2, self.patch_size, self.patch_size).unfold(
            3, self.patch_size, self.patch_size).unfold(4, self.patch_size, self.patch_size)
        unfold_shape = patches.size()
        patches_orig = torch.zeros(unfold_shape[0], 2, unfold_shape[2], unfold_shape[3],
                                   unfold_shape[4], unfold_shape[5], unfold_shape[6], unfold_shape[7])
        for i in range(unfold_shape[2]):
            for j in range(unfold_shape[3]):
                for k in range(unfold_shape[4]):
                    patches_orig[:, :, i, j, k, :, :, :] = self.VoxelCNN(
                        patches[:, :, i, j, k, :, :, :])
        ...
        patches_orig = patches_orig.permute(
            0, 1, 2, 5, 3, 6, 4, 7).contiguous()
        patches_orig = patches_orig.view(
            unfold_shape[0], 2, output_d, output_h, output_w)
        return patches_orig
...
def index_lr(group, d, h, w):  # generate the index of the value that will be assigned to 0
    assign_zeros = [None,
                    None,
                    [np.arange(0, d, 1), np.arange(
                        1, h, 2), np.arange(1, w, 2)],
                    None,
                    [np.arange(1, d, 2), np.arange(
                        0, h, 2), np.arange(1, w, 2)],
                    [np.arange(1, d, 2), np.arange(
                        1, h, 2), np.arange(0, w, 1)],
...
def train(use_cuda, batch_size, low_res, max_epochs, group, output_model, dataset_path, valid_loss_min, model,
    ...
    print('Start the training for group: ', group,
          ' with lower resoltuion is ', low_res)
    for epoch in range(start_epoch, max_epochs):
        for batch_idx, x in enumerate(training_generator):
            ...
            if (group == 0):
                ...
                input = input[:, :, index[0][:, None, None],
                              index[1][:, None], index[2]]

                _, _, ld, lh, lw = input.shape
                index_0 = index_lr(group - 1, ld, lh, lw)
                if (index_0 is not None):
                    input[:, :, index_0[0][:, None, None],
                          index_0[1][:, None], index_0[2]] = 0
                # if(batch_idx==1):
                #    print(input.shape)
                if (group == 5):
            ...
            # predict: shape of input: https://pytorch.org/docs/stable/nn.functional.html#cross-entropy
