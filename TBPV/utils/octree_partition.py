...
def partition_octree_rec(points, bbox_min, bbox_max, level):
    ...
    print("-------begin split octree 获得八个子空间的非空空间,bitstr标明哪些是非空的,bbox标明block大小, retpoint是占用block中点的相对位置------------")
...
def partition_octree(points, bbox_min, bbox_max, level):
    ...
    '''
        np.unique():构建一个递增的唯一值的新列表
            1. return_inverse为True时:返回旧列表中的值在新列表中的索引列表indices
            2. return_count为True时:返回新列表中的值在旧列表中的个数 counts
    '''
    ...
    # print('-----------blockid 只有位置+整除blocksize(量化)----------')
    ...
    # print('---------block_ids_unique 按位置排序+去重--------')
    # print(block_ids_unique, block_ids_unique.shape)
    # print('---------block_idx 返回旧列表中的值在新列表中的索引列表indices--------')
    # print(block_idx)
    # print('---------block_len 新列表中的值在旧列表中的个数--------')
    # print(block_len)
    ...
    for x, y, z in block_ids_unique:
        ...
        # print('------zipparams block_ids_unique的二进制表示---------')
        # print(zip_params)
        sort_key.append(''.join(i + j + k for i, j, k in zip(*zip_params)))
        # print('------sort key---------')
        # print(sort_key)
    sort_idx = np.argsort(sort_key) # 排序+返回排列前对应的索引，以重建原始数据
    # print('------sort_idx---------')
    # print(sort_idx)
    block_ids_unique = block_ids_unique[sort_idx]
    # print('---------block_ids_unique 按拆分二进制的形式排序--------')
    # print(block_ids_unique)
    block_len = block_len[sort_idx]
    # print('---------block_len 对应新的block_ids_unique--------')
    # print(block_len) # 对于测试数据有185个
    # invert permutation
    ...
    # print('--------inv sort idx--------')
    # print(inv_sort_idx)
    # print('---------block_idx 对应新的block_ids_unique--------')
    # print(block_idx)
    ...
    # print('---------local_refs 量化位置 + shape和颜色一样的全0填充--------')
    # print(local_refs)
    # print('---------points_local 位置偏移量 + 颜色--------')
    # print(points_local)
